...
void draw_triangles(const struct Model* model)
{
    ...
    for (i = 0; i < model->n_triangles; ++i) {
        for (k = 0; k < 3; ++k) {
			unsigned int currentTextureIndex = model->triangles[i].points[k].material_index;
			
			glBindTexture(GL_TEXTURE_2D, currentTextureIndex); //should be reverted, and all textures should be combined into one
